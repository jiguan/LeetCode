...
public class SegmentTreeRangeSum {
    ...
    private int query(SegmentTreeNode node, int start, int end) {
        if (node.range[1] < start || node.range[0] > end) {
            return Integer.MIN_VALUE;
        }
        if (node.range[0] >= start && node.range[1] <= end) {
            return node.sum;
        }
        int leftQuery = query(node.left, start, end) == Integer.MIN_VALUE ? 0 : query(node.left, start, end);
        int rightQuery = query(node.right, start, end) == Integer.MIN_VALUE ? 0 : query(node.right, start, end);
