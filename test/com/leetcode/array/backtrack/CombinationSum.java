...
public class CombinationSum {
	...
	private void traverse(int[] candidates, int target, int start, List<Integer> cur, List<List<Integer>> res) {
		if (target == 0) {
	...
	private void sum(int[] candidates, int index, int target, List<Integer> cur, List<List<Integer>> res) {
		for (int i = index; i < candidates.length && candidates[i] <= target; ++i) {
			cur.add(candidates[i]);
			if (target - candidates[i] == 0) {
				res.add(new ArrayList<Integer>(cur));
			} else if (target - candidates[i] > 0) {
				sum(candidates, i, target - candidates[i], cur, res);
			}
			cur.remove(cur.size() - 1);
		}
	}
