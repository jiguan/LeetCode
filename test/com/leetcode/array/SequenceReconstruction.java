...
public class SequenceReconstruction {
    ...
    public boolean sequenceReconstruction(int[] org, List<List<Integer>> seqs) {
        ...
        for (List<Integer> seq : seqs) {
            int prevPosition = -1;
            for (int curr : seq) {
                if (curr >= positions.length || curr < 1 || positions[curr] <= prevPosition) {
                    return false;
                }
    ...
    public void test4() {
        int[] org = {1};
        List<List<Integer>> seqs = Arrays.asList(Arrays.asList(2));
        assertFalse(sequenceReconstruction(org, seqs));
    }
    ...
    public void test5() {
        int[] org = {1};
        List<List<Integer>> seqs =
                Arrays.asList(Arrays.asList(1, -9), Arrays.asList(-9, -8), Arrays.asList(-8, -9));
        assertFalse(sequenceReconstruction(org, seqs));
    }
}
